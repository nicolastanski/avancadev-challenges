...
func home(w http.ResponseWriter, r *http.Request) {
	...
	resultEmail := makeHttpCall("http://localhost:9093")

	if resultEmail.Status == "emailError" {
		result.Status = "error to send email"
	}
...
func makeHttpCall(urlMicroservice string) Result {
	...
	if err != nil {
		result := Result{Status: "Serviço de e-mail fora do ar!"}
		return result
	}
